...
export function FinancialReconciliationModal(
  ...
  const outstanding = Math.max(toMoney(props.totalAmount - props.amountPaid), 0);
  const validation = validateFinancialReconciliation(props);
    ...
    <ModalFrame onClose={props.onClose} title="Edit Payment Details">
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-[#D8C36A]/25 bg-[#D8C36A]/5 p-4">
          ...
          <p className="mt-2 text-sm text-white">Obligation R{toMoney(props.totalAmount).toFixed(2)}</p>
          <p className="text-sm text-white">Paid R{toMoney(props.amountPaid).toFixed(2)}</p>
          <p className="text-sm text-white">Outstanding R{outstanding.toFixed(2)}</p>
          <p className="mt-2 text-xs text-zinc-400">Status {getReconciledPaymentStatus(props.totalAmount, props.amountPaid).replaceAll("_", " ")}</p>
        </div>
      </div>
      <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <MoneyInput label="Booking obligation" value={props.totalAmount} onChange={props.onTotalAmountChange} />
        <MoneyInput label="Amount paid" value={props.amountPaid} onChange={props.onAmountPaidChange} />
      </div>
      ...
      <label className="mt-4 block">
        ...
        <textarea rows={3} value={props.reason} onChange={(event) => props.onReasonChange(event.target.value)} className="w-full rounded-xl border border-white/15 bg-black px-3 py-3 text-white outline-none focus:border-[#D8C36A]" />
      </label>
      {props.error && <p role="alert" className="mt-3 text-sm text-red-200">{props.error}</p>}
      <button type="button" disabled={props.isSaving || Boolean(validation)} onClick={props.onSave} className="mt-5 min-h-12 w-full rounded-full bg-[#D8C36A] px-5 text-sm font-semibold uppercase text-black disabled:cursor-not-allowed disabled:opacity-40">
...
export function GuestCountReconciliationModal(
  ...
  const { booking } = props.details;
  const validation = validateGuestCountReconciliation(props);
    ...
    <ModalFrame onClose={props.onClose} title="Edit Guest Count">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        ...
        <label className="rounded-xl border border-[#D8C36A]/25 bg-[#D8C36A]/5 p-4">
          ...
          <input type="number" min="1" step="1" value={props.guestCount} onChange={(event) => props.onGuestCountChange(Number(event.target.value))} className="mt-2 w-full bg-transparent text-2xl font-semibold text-white outline-none" />
        </label>
      </div>
      ...
      {props.error && <p role="alert" className="mt-3 text-sm text-red-200">{props.error}</p>}
      <button type="button" disabled={props.isSaving || Boolean(validation) || props.guestCount === booking.guestCount} onClick={props.onSave} className="mt-5 min-h-12 w-full rounded-full bg-[#D8C36A] px-5 text-sm font-semibold uppercase text-black disabled:cursor-not-allowed disabled:opacity-40">
