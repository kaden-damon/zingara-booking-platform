-- Phase 39.69: align atomic Floor assignment with the operational booking
-- cohort used by the Floor Queue while preserving every table invariant.
...
begin
  ...
  if v_booking.archived_at is not null
     or v_booking.booking_status::text not in (
       'new',
       'pending_payment',
       'confirmed',
       'checked_in'
