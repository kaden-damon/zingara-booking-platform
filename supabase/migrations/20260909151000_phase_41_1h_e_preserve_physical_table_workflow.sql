-- Phase 41.1H-E follow-up: when temporary capacity is active, compare the
-- complete operational table inventory with the effective zone limit. Because
-- the same temporary seats occur on both sides, physical inventory remains
-- bounded by the configured base while normal physical-table edits continue.
